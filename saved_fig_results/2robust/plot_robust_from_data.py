...
for size in ["1p3", "2p7"]:
    ...
    for l, task in enumerate(["sub", "ist", "dlt"]):
        ...
        save_dir = f"{size}B-{name}-c5-m400-T1000-tempall-alpha{alpha}-True-{task}-nsiuwm-{num}-11.pkl"
        results_dict = pickle.load(open(save_dir, "rb"))
        save_dir1 = f"{size}B-{name}-c5-m400-T1000-tempall-alpha{alpha}-False-{task}-nsiuwm-{num}-11.pkl"
        results_dict1 = pickle.load(open(save_dir1, "rb"))
        save_dir2 = f"{size}B-{name}-c5-m400-T1000-tempall-alpha{alpha}-True1e-3-{task}-nsiuwm-{num}-11.pkl"
        ...
        for i, temp in enumerate(all_tempretures):
            ...
            plt.savefig(f"{size}B-corruption-c{c}-m{m}-T{T}-tempall-mix-all.pdf", dpi=300)
    ...
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 0.995), ncol=len(different_s))
